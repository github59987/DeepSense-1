...
def deepsense_model(is_training, feature_batch, summary_visalization=None):
    ...
    :param feature_batch: placeholder, 一个批次的特征矩阵 （batch_size, 6*2*F）
    ...
    with tf.name_scope('input_dataset'):
        ...
        # print('读入一个批次的特征矩阵维度为: %s' % feature_batch.shape)
        # 按传感器类比分为三组数据
        input_acc, input_gry = tf.split(value=feature_batch, num_or_size_splits=2, axis=1)
        # print('各个传感器一个批次的数据维度为: %s %s' % (input_acc.shape, input_gry.shape))
    ...
    with tf.name_scope('cnn_part1'):
        ...
        # acc部分
        cnn_acc_1 = CNN(x=None, w_conv=conv_para_1['w1'], stride_conv=None, stride_pool=None)
        ...
        conv1_acc = sub_conv_bn_dropout(CNN_object=cnn_acc_1, op=input_acc_reshape, is_training=is_training,
                                        strides=[1, 1, CONV_LEN_1, 1])  # (batch_size, :, :, OUT_NUM)

        cnn_acc_2 = CNN(x=None, w_conv=conv_para_1['w2'], stride_conv=None, stride_pool=None)
        conv2_acc = sub_conv_bn_dropout(CNN_object=cnn_acc_2, op=conv1_acc, is_training=is_training,
                                        strides=[1, 1, CONV_LEN_2, 1])  # (batch_size, :, :, OUT_NUM)

        cnn_acc_3 = CNN(x=None, w_conv=conv_para_1['w3'], stride_conv=None, stride_pool=None)
        conv3_acc = sub_conv_bn_dropout(CNN_object=cnn_acc_3, op=conv2_acc, is_training=is_training,
                                        strides=[1, 1, CONV_LEN_3, 1])  # (batch_size, :, :, OUT_NUM)

        # gry部分
        cnn_gry_1 = CNN(x=None, w_conv=conv_para_1['w1'], stride_conv=None, stride_pool=None)
        ...
        cnn_gry_2 = CNN(x=None, w_conv=conv_para_1['w2'], stride_conv=None, stride_pool=None)
        conv2_gry = sub_conv_bn_dropout(CNN_object=cnn_gry_2, op=conv1_gry, is_training=is_training,
                                        strides=[1, 1, CONV_LEN_2, 1])  # (batch_size, :, :, OUT_NUM)

        cnn_gry_3 = CNN(x=None, w_conv=conv_para_1['w3'], stride_conv=None, stride_pool=None)
        conv3_gry = sub_conv_bn_dropout(CNN_object=cnn_gry_3, op=conv2_gry, is_training=is_training,
                                        strides=[1, 1, CONV_LEN_3, 1])  # (batch_size, :, :, OUT_NUM)

        # flat、concat节点conv3_acc和conv3_gry
        ...
        # print('第一部分卷积层输出单个批次维度为: %s' % cnn_concat1.shape)  # shape=(batch_size, 2, :, 1)
    ...
    with tf.name_scope('cnn_part2'):
        ...
        cnn_merge_1 = CNN(x=None, w_conv=conv_para_2['w1'], stride_conv=None, stride_pool=None)
        conv1_merge = sub_conv_bn_dropout(CNN_object=cnn_merge_1, op=cnn_concat1, is_training=is_training,
                                          strides=[1, 1, CONV_MEG_1, 1])  # (batch_size, :, :, OUT_NUM)

        cnn_merge_2 = CNN(x=None, w_conv=conv_para_2['w2'], stride_conv=None, stride_pool=None)
        conv2_merge = sub_conv_bn_dropout(CNN_object=cnn_merge_2, op=conv1_merge, is_training=is_training,
                                          strides=[1, 1, CONV_MEG_2, 1])  # (batch_size, :, :, OUT_NUM)

        cnn_merge_3 = CNN(x=None, w_conv=conv_para_2['w3'], stride_conv=None, stride_pool=None)
        cnn_fin = sub_conv_bn_dropout(CNN_object=cnn_merge_3, op=conv2_merge, is_training=is_training,
                                      strides=[1, 1, CONV_MEG_3, 1])  # (batch_size, :, :, OUT_NUM)
        # print('第二部分卷积层输出单个批次维度为: %s' % cnn_fin.shape)
        # 对卷积层的输出在后三个维度上进行flat
        axis_cnn = cnn_fin.get_shape().as_list()
        cnn_flat = tf.reshape(tensor=cnn_fin, shape=(-1, axis_cnn[1] * axis_cnn[2] * axis_cnn[3]))  # (batch_size, :)
        print('第二部分卷积层输出大小为: %s' % cnn_flat.shape)
    with tf.name_scope('rnn'):
        # 对第二部分卷积输出做维度转换，转换后为:(batch_size, 10, :)
        cnn_output = tf.reshape(tensor=cnn_flat, shape=(-1, 10, cnn_flat.get_shape().as_list()[-1]//10))  # (batch_size, T, :)
        # multiGRU部分
        gru = RNN(x=cnn_output, max_time=10, num_units=128)
...
def model_training(training_time):
    ...
    with ds_graph.as_default():
        ...
        with tf.name_scope('placeholder'):
            feature_batch = tf.placeholder(shape=(BATCH_SIZE, 12000), dtype=tf.float32, name='feature_batch')
